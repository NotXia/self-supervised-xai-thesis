...
\paragraph{Compactness} Compactness measures the extent to which an attribution map is concise and concentrates relevance on a small subset of the input features. In its original work \cite{sparsity_metric}, this metric is called sparsity. However, its definition in \citeauthor{sparsity_metric} \cite{sparsity_metric} is somewhat counterintuitive to interpret as a higher sparsity indicate a more compact attribution map. Therefore, we decided to denote the metric as compactness in the rest of the work.
Formally, given an attribution score $S$, compactness is computed as:
\begin{equation}
    \texttt{compactness} = \frac{S_\text{max}}{S_\text{mean}},
\end{equation}
...
Higher compactness indicates that the explanation is more concentrated in specific regions, reducing visual clutter and improving interpretability. On the contrary, lower compactness indicates that many regions of the attribution map are active which might make it more difficult to understand.
